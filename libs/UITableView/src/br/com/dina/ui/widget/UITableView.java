...
public class UITableView extends RelativeLayout {
	...
	public void addItem(String title, String summary,boolean bChevon) {
		Item item  = new Item(title, summary);
		item.isChvon = bChevon;
		mItemList.add(item);
	...
	public void editItem(int index,String title,String summary,int summaryColor){
		Item item  = mItemList.get(index);
		if(item == null)
			return;
		item.setTitle(title);
		item.setSummary(summary);
		item.summaryColor = summaryColor;
		;
		setupItem(mListContainer.getChildAt(index),item,index);
	}
	/**
	 * 
	 * @param view
	...
	private void setupItem(View view, Item item, int index) {
		...
		if(item.getSummary() != null) {
			...
			((TextView) view.findViewById(R.id.subtitle)).setTextColor(item.summaryColor);
		}
		else {
